...
df_ventas = pd.read_excel(r'datasets/Ventas.xlsx', sheet_name='Detalle')
df_ventas_acum = pd.read_excel(r'datasets/Ventas.xlsx', sheet_name='Acumulado')

# FASE 3: CREACIÓN GRÁFICO GEOGRÁFICO
mapbox_access_token = "pk.eyJ1IjoiZ2Vyb2JlbmEiLCJhIjoiY2trbGdqM3Y0MXJrcjJubGFkZWw1bnJ2MiJ9.J9lyyC0Q9HplIBoCfvnLiw"
...
app.layout = html.Div([
                    ...
                    dcc.DatePickerRange(id='selector_fecha',
                                        start_date = df_ventas['Fecha compra'].min(),
                                        end_date = df_ventas['Fecha compra'].max()
                    )
...
#Para que se enlace el hoverdata con el grafico anterior
def actualizar_graph_cat(fecha_min, fecha_max, seleccion,hoverData):
# FASE 5: Interactividad inter-gráfico hoverData
    v_index = hoverData['points'][0]['x'] # Esta linea es para escoger el valor del dato por donde esta pasando el mouse
...
# FASE 2: CREACIÓN DE GRÁFICOS Y GROUPBY (considerar que tenemos un dataframe ya filtrado filtered_df)
    ...
    return{
        'data': [go.Bar(
            x= df_agrupado['Categoría'],
            y = df_agrupado['Beneficio'])],
        
        'layout':go.Layout(
            title="¿Cuales han sido los beneficios de cada categoria",
